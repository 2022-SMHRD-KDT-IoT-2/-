...
<body>
 ...
 %> <a class="list-a"
						href="productlist.jsp?pageNum=<%=startPage + 10%>">[다음]</a> <%
 }
...
%>
			<a href="productlist.jsp?pageNum=<%=startPage - 10%>&sel=<%=sel%>&find=<%=find%>">[이전]</a>	
<%			
		}
...
%>
			<a href="productlist.jsp?pageNum=<%=i%>&sel=<%=sel%>&find=<%=find%>">[<%=i %>]</a>
<%				
			}
		}
		...
		if(endPage < pageCount){
%>
			<a href="productlist.jsp?pageNum=<%=startPage + 10 %>&sel=<%=sel%>&find=<%=find%>">[다음]</a>
<%			
		}
	}
%>
